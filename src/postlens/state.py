...
class RecurrentStateStore:
    """In-process cache of recurrent states keyed by `skill_id` + content digest.

    Process-local: states do not persist across runs in v0.1.0. v0.1.1 will
    add safetensors-based on-disk persistence (no pickle).
    """
    ...
    @staticmethod
    def _digest(skill_id: str, state: BackboneState) -> str:
        h = hashlib.sha256()
